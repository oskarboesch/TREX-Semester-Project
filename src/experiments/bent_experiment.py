...
def _bent_eval_sets(test_log_names, folder_name: str) -> List[EvalSet]:
    """Build evaluation datasets specific to the paper experiment."""
    return [
        ("bent test", lambda: (test_log_names, folder_name)),
        ("paper", ld.get_paper_logs),
        ("anatomical", ld.get_anat_logs),
        ("conical no clot", lambda: ld.get_conical_logs(bent=True, with_clot=False)),
    ]


def bent_data_experiment(data_cfg_path):
    run_experiment(
        experiment_name="bent",
        data_cfg_path=data_cfg_path,
        train_logs_fn=lambda: ld.get_conical_logs(bent=True),
        eval_sets_builder=_bent_eval_sets,
    )

def bent_data_experiments_all():
    ...
    for data_cfg_path in data_cfg_paths:
        bent_data_experiment(data_cfg_path)

    print("="*10 + "All bent data experiments completed."+ "="*10)