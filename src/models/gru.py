...
def load_gru_model(model_path, input_size, model_cfg, device):
    # create model instance with SAME parameters used during training
    model = GRUClassifier(
        ...
        with_images=model_cfg.get("with_images", False)
    ).to(device)
